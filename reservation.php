        ...
        if (empty($formData['fromCity']) || empty($formData['toCity']) || 
            ...
            try {
                ...
                $stmt = $db->prepare("
                    ...
                    FROM prices 
                    WHERE from_city_id = (SELECT id FROM cities WHERE name = :fromCity)
                    AND to_city_id = (SELECT id FROM cities WHERE name = :toCity)
                ");
                ...
                if (!$priceData) {
                    ...
                    if ($stmt->execute()) {
                        $success = "Votre réservation a été enregistrée avec succès. Le prix total est de " . number_format($totalPrice, 2) . " TND.";
                                ...
                                <div class="col-md-6">
                                    <div class="form-check card p-3">
                                        <input class="form-check-input" type="radio" name="louageType" id="standardLouage" 
                                               value="standard" <?php echo $formData['louageType'] === 'standard' ? 'checked' : ''; ?>>
